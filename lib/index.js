...
function buildFields(schema) {
  return fieldsFor(schema).map(field => field.substring(1));
}
...
function fieldsFor(object, context) {
  context = context || '';
  var fields = [];
  var keys = Object.keys(object);
  var primitiveFields = keys.filter(field => !object[field].type.startsWith('object'))
  var complexFields = keys.filter(field => object[field].type.startsWith('object'))

  primitiveFields.forEach(field => {
    if (isArrayType(object[field].type)) {
  ...
  complexFields.forEach(field => {
    if (isArrayType(object[field].type)) {
      fields = fields.concat(fieldsFor(object[field].values, `${context}.${field}[]`));
    } else {
      fields = fields.concat(fieldsFor(object[field].values, `${context}.${field}`));
    }
  })
